...
def plot_seasonality(seasonality_df):
    ...
    st.write("Aperçu seasonality_df")
    st.dataframe(seasonality_df.head(20))
    st.write(seasonality_df.dtypes)
    st.write("Stats quantity", seasonality_df["quantity"].describe())
...
def plot_family_distribution(grouped_df, selected_families):
    ...
    for i, fam in enumerate(selected_families):
        fam_data = (
            grouped_df[grouped_df["family"] == fam]
        ...
        with cols[i]:
            ...
            fig = px.bar(fam_data, x="product_label", y="quantity", title=None)
            fig.update_layout(
                xaxis_title=None, yaxis_title="Quantité", margin=dict(t=10)
            )
            st.plotly_chart(fig, use_container_width=True)


def plot_predictions_vs_truth(
    df_eval, y_col="quantity", pred_col="prediction", family_name=None
):
    """
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df_eval[pred_col],
            mode="lines+markers",
            name="Prévisions modèle",
        )
    )

    fig.update_layout(
        title=f"Prédictions vs Réalité – {family_name}"
        if family_name
        else "Prédictions vs Réalité",
        xaxis_title="Date",
...
def plot_product_graph(G, color_map=None):
    ...
    color_lookup = {
        com: f"rgba{tuple(int(255 * c) for c in colormap(i)[:3]) + (0.9,)}"
    ...
    node_trace = go.Scatter(
        ...
        marker=dict(
            color=node_colors, size=15, line=dict(width=2, color="DarkSlateGrey")
        ),
        hoverinfo="text",
    ...
    if color_map:
        for i, (com, color) in enumerate(color_lookup.items()):
            annotations.append(
                dict(
                    x=1.02,
                    y=1 - (i * 0.1),  # Ajuster la position de la légende
                    xref="paper",
                    yref="paper",
